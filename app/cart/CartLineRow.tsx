...
export default function CartLineRow({ product, qty }: { product: Product; qty: number }) {
  ...
  return (
    <div className="flex gap-4 p-4 rounded-2xl border border-[var(--color-line)] bg-[var(--color-bg-soft)]/40">
      ...
      <div className="flex-1">
        <Link href={`/shop/${product.slug}`} className="font-display text-lg">
        ...
        <div className="mt-3 flex items-center justify-between flex-wrap gap-2">
          ...
          <div className="font-display text-xl text-[var(--color-gold-bright)]">{money(product.price_cents * qty)}</div>
        </div>
      </div>
      <button
        aria-label="Remove"
        onClick={() => start(() => removeFromCartAction(product.id))}
        className="self-start text-[var(--color-ink-dim)] hover:text-[var(--color-wine-bright)] p-2"
      >
        <X size={16} />
      </button>
    </div>
